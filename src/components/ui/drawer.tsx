...
function DrawerContent({
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      ...
      >
        {/*
         * Every visual utility here needs `!`: vaul styles `[data-vaul-handle]`
         * from a stylesheet it injects after ours, and an attribute selector ties
         * a single-class utility on specificity, so source order hands vaul the
         * win. Without these it silently renders its own 32x5px hardcoded
         * #e2e2e4 pill (ignoring the theme) and its `display:block` defeats
         * `hidden`, so the grabber also appears on left/right/top drawers.
         */}
        <DrawerPrimitive.Handle
          data-slot="drawer-handle"
          className="mx-auto mt-4 hidden! h-1! w-[100px]! shrink-0 rounded-full! bg-muted! opacity-100! group-data-[vaul-drawer-direction=bottom]/drawer-content:block!"
        />
        {children}
      </DrawerPrimitive.Content>
    </DrawerPortal>
